...
agarMap.init = function init() {
    ...
    $('#mapContainer').append($('<div id="coords"></div>')
        .css('display', 'none'));
    $('#mapContainer').append($('<a href="#">details</a>')
       .click(function(){
           $('#coords').slideToggle();
           ...
           return false; //prevent native link behavior
    }));
};
...
agarMap.processMessage16 = function processMessage16(dataView) {
    ...
    while(true) {
        ...
        blob.x = dataView.getInt16(offset, true); offset += 2;
        blob.y = dataView.getInt16(offset, true); offset += 2;
        blob.size = dataView.getInt16(offset, true); offset += 2;
        // Parse color (base 16)
